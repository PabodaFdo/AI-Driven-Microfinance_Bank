...
export default function App() {
  const { token, user, authReady, loginSuccess, logout } = useAuth();
  ...
  // ✅ Wait for auth hydration to complete before rendering
  if (!authReady) {
    return (
      <div style={{
        display: 'grid',
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 24, fontWeight: 700, color: C.ink, marginBottom: 12 }}>Loading...</div>
          <div style={{ width: 40, height: 40, border: `2px solid ${C.border}`, borderTop: `2px solid ${C.royalBlue}`, borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto' }} />
        </div>
      </div>
    );
  }
