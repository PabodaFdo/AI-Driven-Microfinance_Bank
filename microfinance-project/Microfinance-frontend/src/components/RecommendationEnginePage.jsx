          ...
          {recommendation ? (
            <div>
              <div style={{
                ...
                {recommendation.inputSnapshot?.aiRecommendation?.shap_explanations && (
                  <ShapExplanationChart
                    shapExplanations={recommendation.inputSnapshot.aiRecommendation.shap_explanations}
                  />
                )}
              ...
              <div style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20 }}>
                  {Object.entries(recommendation.inputSnapshot).map(([key, value]) => {
                    // Format AI Recommendation object nicely
                    if (key === 'aiRecommendation' && typeof value === 'object' && value !== null) {
                      return (
                        <div key={key}>
                          ...
                          <div style={{ fontSize: 14, color: C.ink, lineHeight: 1.6 }}>
                            <div>💰 Amount: <strong>{formatCurrency(value.recommended_amount)}</strong></div>
                            <div>📅 Term: <strong>{value.recommended_term} months</strong></div>
                            <div>📊 Rate: <strong>{value.recommended_rate}%</strong></div>
                            <div>✓ Status: <strong>{value.feasibility}</strong></div>
                          </div>
                        </div>
                      );
                    }
