...
public class ApplicantService {
    ...
    public ApplicantRegistrationResponse registerApplicant(ApplicantRegistrationRequest request) {
        ...
        // Extract age from NIC (Phase 1 - AI model requirement)
        extractAndSetAge(applicant);
    ...
    /**
     * Extract age from NIC and set it on the applicant (Phase 1 - AI model requirement)
     * Throws BusinessException if age cannot be derived from NIC or is invalid
     * @param applicant Applicant to set age on
     * @throws BusinessException if NIC is invalid or age cannot be derived
     */
    private void extractAndSetAge(Applicant applicant) {
        Integer age = NICParser.extractAge(applicant.getNic());

        if (age != null && age > 0 && age < 150) {
            applicant.setAge(age);
        } else {
            throw new BusinessException("Invalid NIC: cannot derive a valid age.");
        }
    }
    ...
    private Applicant mapToEntity(ApplicantRequest req, Applicant a) {
        ...
        a.setEducation(req.getEducation());
