...
public class RecommendationService {
    ...
    public RecommendationResponse generateRecommendation(String applicationId) {
        ...
        try {
            ...
            // Try to use AI model for recommendations
            Map<String, Object> aiResult = null;
            try {
                aiResult = aiServiceClient.predictRecommendation(applicant, application);
            } catch (Exception e) {
                System.out.println("⚠ AI model call failed for recommendation, using rule-based: " + e.getMessage());
            }

            // If AI model provided a recommendation, use it
            if (aiResult != null) {
                try {
                    System.out.println("DEBUG: AI Response received: " + aiResult);
                    recommendation.setModelVersion("AI_MODEL_REC_V1");

                    // Extract AI recommendations
                    Object aiAmount = aiResult.get("recommended_amount");
                    Object aiTerm = aiResult.get("recommended_term");
                    Object aiRate = aiResult.get("recommended_rate");
                    Object aiFeasibility = aiResult.get("feasibility");
                    Object aiReasoning = aiResult.get("reasoning");
                    Object aiShapExplanations = aiResult.get("shap_explanations");

                    System.out.println("DEBUG: Extracted - Amount: " + aiAmount + ", Term: " + aiTerm + ", Rate: " + aiRate);

                    if (aiAmount instanceof Number) {
                        recommendation.setRecommendedAmount(((Number) aiAmount).doubleValue());
                    }
                    if (aiTerm instanceof Number) {
                        recommendation.setRecommendedPeriod(((Number) aiTerm).intValue());
                    }
                    if (aiRate instanceof Number) {
                        recommendation.setRecommendedInterestRate(((Number) aiRate).doubleValue());
                    }
                    if (aiFeasibility != null) {
                        recommendation.setFeasibility(aiFeasibility.toString());
                    }
                    if (aiReasoning != null) {
                        recommendation.setReasoning(aiReasoning.toString());
                    }

                    // Store AI response in input snapshot
                    recommendation.getInputSnapshot().put("aiRecommendation", aiResult);

                    System.out.println("✓ Using AI model recommendation for application: " + applicationId);
                } catch (Exception e) {
                    System.out.println("⚠ Error processing AI recommendation: " + e.getMessage());
                    e.printStackTrace();
            ...
            System.out.println("✓ Recommendation generated for application " + applicationId +
                    ...
                    ", Model: " + recommendation.getModelVersion() +
